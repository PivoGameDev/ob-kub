...
$canonical = 'https://ob-kub.ru/catalog/dairy/';
...
$categories = [
    ...
    ['key' => 'storage', 'emoji' => '🏭'],
    ['key' => 'vdp', 'emoji' => '🌡️'],
    ['key' => 'fermentation', 'emoji' => '🧫'],
    ['key' => 'cheese-maker', 'emoji' => '🧀'],
    ['key' => 'cottage-cheese', 'emoji' => '🥄'],
    ['key' => 'yeast', 'emoji' => '🔬'],
    ['key' => 'cheese-shelves', 'emoji' => '🧀', 'custom' => true],
];
$h1 = $dairyCategory['h1'];
$metaTitle = $dairyCategory['title'];
$metaDesc = $dairyCategory['desc'];
...
<span class="current">Молочное оборудование</span>
</div>
<div class="hero-text">
...
<?php foreach ($categories as $c):
    if (!empty($c['custom'])) {
        ...
        $minVol = 0; $maxVol = 0;
        if ($volCount) { $minVol = min($dk['volumes']); $maxVol = max($dk['volumes']); }
        $countStr = 'от 2 до 12 секций';
    } else {
        $dk = $dairyData[$c['key']];
        $volCount = count($dk['volumes']);
...
?>
<a href="/catalog/dairy/<?= $c['key'] ?>/" class="cat-card">
